...
@section('title', 'Certificaciones y Proyectos - BET CAPITAL SAC')
...
@section('content')
    ...
    <section class="relative h-[480px] flex items-center overflow-hidden bg-primary hero-diagonal">
        ...
        <div class="relative z-20 w-full max-w-[1440px] mx-auto px-margin text-white -mt-10">
            <div class="max-w-2xl">
                ...
                <h1 class="font-headline-xl text-headline-xl text-white mb-md">{{ $page->hero_title ?? 'Certificaciones' }}</h1>
    ...
    @if ($certifications->count() > 0)
        <section class="px-margin max-w-[1440px] mx-auto py-16">
            <div
                class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6 pb-6 border-b border-outline-variant scroll-reveal">
                <div class="space-y-2 w-full md:w-1/2">
                    <span
                        class="text-transit-blue font-label-sm text-label-sm uppercase tracking-widest font-semibold">Acreditación</span>
                    <h2 class="font-headline-lg text-headline-lg text-ocean-deep">Homologaciones del Sector</h2>
                </div>
                <p
                    class="font-body-md text-body-md text-on-surface-variant w-full md:w-1/2 lg:max-w-md text-sm leading-relaxed">
                    Mantenemos nuestros estándares operativos auditados y certificados para garantizar la seguridad de su
                    carga en puertos y almacenes nacionales.
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach ($certifications as $cert)
                    <div
                        class="bg-surface-container-lowest border border-outline-variant p-4 flex flex-col hover:border-transit-blue transition-all duration-300 group rounded-sm scroll-reveal zoom-in">
                        <div class="aspect-[3/4] bg-surface-container-low overflow-hidden relative rounded-sm mb-4">
                            @if (!empty($cert->image))
                                <img class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500"
                                    src="{{ str_starts_with($cert->image, 'http') ? $cert->image : asset('storage/' . $cert->image) }}"
                                    alt="{{ $cert->title }}" />
                            @endif
                        ...
                        @if (!empty($cert->pdf_file))
                            <a href="{{ asset('storage/' . $cert->pdf_file) }}" target="_blank"
                                class="mt-4 flex items-center justify-center gap-2 bg-surface-container-low text-transit-blue py-2 rounded-sm font-label-bold text-label-bold hover:bg-transit-blue hover:text-white transition-all duration-300 text-xs font-bold w-full">
            ...
            <div class="px-margin max-w-[1440px] mx-auto">
                <div class="grid grid-cols-12 gap-6 mb-12 pb-6 border-b border-white/10 scroll-reveal">
                    <div class="col-span-12 lg:col-span-6 space-y-2">
                        <span
                            class="text-electric-cyan font-label-sm text-label-sm uppercase tracking-widest font-semibold">Portafolio
                            en Acción</span>
                        ...
                        <p class="font-body-md text-body-md opacity-85 text-sm">
                            Resumen visual de nuestra capacidad logística. Desde el transporte de maquinaria pesada hasta
                            traslados portuarios prioritarios.
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach ($projects as $project)
                        <div
                            class="group relative overflow-hidden rounded-sm bg-slate-800 shadow-md aspect-video md:aspect-[4/3] scroll-reveal zoom-in">
                            ...
                            @endif
                            <div
                                class="absolute inset-0 bg-gradient-to-t from-ocean-deep/95 via-ocean-deep/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
