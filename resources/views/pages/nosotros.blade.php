...
<section class="relative h-[50vh] min-h-[400px] flex items-center justify-center overflow-hidden bg-ocean-deep">
    ...
    <div class="relative z-10 text-center px-margin max-w-5xl text-white scroll-reveal zoom-in visible">
        <h1 class="font-headline-xl text-headline-xl text-white">
            {!! str_replace('Crecimiento', '<span class="text-electric-cyan border-b-2 border-electric-cyan/30 pb-1">Crecimiento</span>', $page->hero_title) !!}
        </h1>
...
<section class="py-16 px-margin max-w-[1440px] mx-auto">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        @if(!empty($page->section_data['mission_title']))
            <div class="lg:col-span-5 bg-surface-container-lowest border border-outline-variant p-6 flex flex-col justify-between hover:border-transit-blue hover:shadow-md transition-all duration-300 rounded-sm scroll-reveal slide-left">
                <div class="space-y-4">
                    <div class="w-12 h-12 bg-surface-container-low rounded-sm flex items-center justify-center text-transit-blue">
                        <span class="material-symbols-outlined text-2xl">{{ $page->section_data['mission_icon'] ?? 'track_changes' }}</span>
                    </div>
                    <h3 class="font-title-md text-title-md text-ocean-deep font-semibold">{{ $page->section_data['mission_title'] }}</h3>
        ...
        @if(!empty($page->section_data['team_image']))
            <div class="lg:col-span-7 h-[360px] relative overflow-hidden rounded-sm shadow-sm scroll-reveal slide-right">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-6 text-white">
                    ...
                    <h4 class="font-headline-lg text-headline-lg text-white font-bold">{{ $page->section_data['team_title'] ?? 'Equipo Profesional Directivo' }}</h4>
                </div>
            </div>
        @endif
        ...
        @if(!empty($page->section_data['vision_title']))
            <div class="lg:col-span-12 bg-ocean-deep text-white p-12 rounded-sm shadow-sm scroll-reveal zoom-in">
                <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
                    <div class="w-14 h-14 bg-white/10 rounded-sm flex items-center justify-center shrink-0 text-electric-cyan">
                        <span class="material-symbols-outlined text-3xl">{{ $page->section_data['vision_icon'] ?? 'visibility' }}</span>
                    </div>
                    <div class="space-y-2">
                        <h3 class="font-title-md text-title-md text-electric-cyan font-semibold">{{ $page->section_data['vision_title'] }}</h3>
        ...
        @if(!empty($page->section_data['team_description']))
            <div class="lg:col-span-12 bg-surface-container-low border border-outline-variant p-6 rounded-sm scroll-reveal">
                <h4 class="font-title-md text-title-md text-ocean-deep mb-2 font-semibold">Garantía Operativa y de Seguridad</h4>
                <p class="font-body-md text-body-md text-on-surface-variant leading-relaxed">
